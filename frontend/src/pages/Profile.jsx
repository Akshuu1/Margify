            ...
            <div className="flex-1 w-full">
              ...
              {!showPasswordChange ? (
                <div className="flex flex-col gap-3">
                  <button onClick={() => setShowPasswordChange(true)} className="w-full bg-[#2f2f2f] hover:bg-[#3f3f3f] text-[#e0e0e0] py-3.5 rounded-xl transition-all duration-300 font-bold text-sm flex items-center justify-center gap-2 border border-white/5"><Lock size={16} />Change Password</button>
                  <div className="flex gap-3">
                    <button onClick={() => navigate("/search")} className="flex-1 bg-white/5 hover:bg-white/10 text-[#e0e0e0] py-3.5 rounded-xl transition-all duration-300 font-bold text-sm flex items-center justify-center gap-2"><Search size={16} />Find Routes</button>
                    <button onClick={handleLogout} className="flex-1 bg-red-500/10 hover:bg-red-500/20 text-red-400 py-3.5 rounded-xl transition-all duration-300 font-bold text-sm flex items-center justify-center gap-2 border border-red-500/10"><LogOut size={16} />Sign Out</button>
                  </div>
                </div>
              ) : (
                <form onSubmit={handleChangePassword} className="bg-[#2a2a2a] p-6 rounded-2xl border border-white/10 animate-in fade-in slide-in-from-top-4 duration-300">
                  <h3 className="text-white font-bold mb-4 flex items-center gap-2"><Lock size={18} className="text-[#FFCB74]" />Change Password</h3>
                  {message.text && (
                  ...
                  <div className="space-y-4 mb-6">
                    <div>
                      ...
                      <input type="password" value={passwords.old} onChange={e => setPasswords({ ...passwords, old: e.target.value })} className="w-full bg-[#1c1c1c] border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#FFCB74] transition-colors" placeholder="••••••••" required />
                    </div>
                    <div>
                      <label className="block text-xs text-[#888] mb-1.5 uppercase font-bold tracking-wider">New Password</label>
                      <input type="password" value={passwords.new} onChange={e => setPasswords({ ...passwords, new: e.target.value })} className="w-full bg-[#1c1c1c] border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#FFCB74] transition-colors" placeholder="••••••••" required />
                    </div>
                    <div>
                      <label className="block text-xs text-[#888] mb-1.5 uppercase font-bold tracking-wider">Confirm New Password</label>
                      <input type="password" value={passwords.confirm} onChange={e => setPasswords({ ...passwords, confirm: e.target.value })} className="w-full bg-[#1c1c1c] border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#FFCB74] transition-colors" placeholder="••••••••" required />
                    </div>
                  </div>
                  <div className="flex gap-3">
                    <button type="button" onClick={() => setShowPasswordChange(false)} className="flex-1 bg-[#3f3f3f] hover:bg-[#4f4f4f] text-white py-3 rounded-xl font-bold transition-colors">Cancel</button>
                    <button type="submit" className="flex-1 bg-[#FFCB74] hover:bg-[#eebb55] text-[#111111] py-3 rounded-xl font-bold transition-colors shadow-lg shadow-[#FFCB74]/20">Update Password</button>
                  </div>
                </form>
              )}
            </div>
