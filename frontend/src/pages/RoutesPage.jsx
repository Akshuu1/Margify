...
export function RoutesPage() {
  ...
  useEffect(() => {
    console.log(`[RoutesPage ${PAGE_V}] Component Mount. Source:`, source?.name);
    ...
    async function fetchRoutes() {
      ...
      try {
        ...
        console.log(`[RoutesPage] Data Received:`, {
          routes: data.routes?.length,
          tourist: data.touristPlaces?.length,
          weather: !!data.weather,
          pitstops: !!data.hubPitStops
        });
        ...
        console.error("[RoutesPage] Fetch Error:", err);
      } finally {
        setLoading(false);
      }
    }
  ...
  if (loading) {
    return <div className="w-screen h-screen flex justify-center items-center text-[#e0e0e0] font-[Space Grotesk]">Finding routes...</div>;
  }

  if (!source || !destination) {
    return <div className="w-screen h-screen flex justify-center items-center text-[#e0e0e0] font-[Space Grotesk]">No route data found.</div>;
  }
  ...
  return (
    <div className="w-screen h-screen flex flex-col text-[#e0e0e0] overflow-hidden" style={{ fontFamily: "Space Grotesk" }}>
        ...
        <div className="bg-[#FFCB74] rounded-2xl p-4 text-[#111111] shadow-xl">
          <div className="flex flex-col xl:flex-row items-center gap-4 xl:gap-8">
            ...
            <div className="w-full xl:flex-1 flex flex-col md:flex-row items-center gap-3">
              <div className="flex-1 w-full bg-[#2f2f2f] text-[#e0e0e0] px-5 py-3 rounded-xl text-center shadow-inner font-medium text-sm md:text-base border border-white/5 truncate">
                {source.name}
              </div>
            ...
            <div className="flex gap-2 w-full xl:w-auto">
              <button onClick={() => window.location.href = '/search'} className="flex-1 xl:flex-none px-4 py-3 bg-[#111111] text-white rounded-xl font-bold flex items-center justify-center gap-2 text-xs transition-transform active:scale-95">
                Change Route
              </button>
      ...
      <div className="flex-1 overflow-y-auto px-4 sm:px-6 md:px-12 lg:px-16 pb-12 scroll-smooth">
        ...
        {/* Unified Insight & Intelligence Hub */}
        <DiscoveryHub
          weather={weather}
          touristPlaces={touristPlaces}
          hubPitStops={hubPitStops}
          onShowTourist={() => setShowTouristPlaces(true)}
          onShowFood={() => setShowFoodHubs(true)}
        />
