    ...
    try {
        ...
        if (places && places.length > 0) {
            const calculateScore = (p) => {
                ...
                // Heavily reward major hubs, interchanges and terminals
                const majorHubKeywords = ['terminal', 'isbt', 'stand', 'junction', 'interchange', 'hub', 'central', 'main', 'gate'];
                majorHubKeywords.forEach(keyword => {
                    if (name.includes(keyword)) score -= 100; // Equivalent to being ~10km closer
                });

                if (isSonipatArea && name.includes('sonipat')) score -= 60;
